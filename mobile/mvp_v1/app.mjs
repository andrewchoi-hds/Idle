...
const DEFAULT_COLLECTION_GUARDIAN_ROWS = [
  { guardian_id: "gdn_001", name_ko: "청운학", rarity: "rare", effect_type: "battle_speed_pct", note: "전투 속도 보조", acquire_pool: "guardian_core" },
  { guardian_id: "gdn_002", name_ko: "현갑령", rarity: "rare", effect_type: "death_fail_pct", note: "도겁 사망률 완화", acquire_pool: "guardian_core" },
  { guardian_id: "gdn_003", name_ko: "진기동자", rarity: "rare", effect_type: "offline_efficiency_pct", note: "오프라인 품질 보정", acquire_pool: "guardian_core" },
];
const DEFAULT_COLLECTION_RELIC_ROWS = [
  { relic_id: "rlc_001", name_ko: "정기잔향", rarity: "rare", effect_type: "battle_start_qi_pct", note: "전투 시작 기 보조", acquire_pool: "relic_core" },
  { relic_id: "rlc_002", name_ko: "호천패", rarity: "rare", effect_type: "minor_fail_weight_pct", note: "경상 실패 가중치 완화", acquire_pool: "relic_core" },
  { relic_id: "rlc_003", name_ko: "귀맥석", rarity: "rare", effect_type: "auto_breakthrough_warmup_sec", note: "자동돌파 워밍업 단축", acquire_pool: "relic_core" },
];
const DEFAULT_COLLECTION_DUPLICATE_ROWS = [
  { source_kind: "guardian", rarity: "rare", duplicate_count_required: "1", yield_currency_value: "8", secondary_currency_value: "0", pity_progress_value: "1" },
  { source_kind: "guardian", rarity: "epic", duplicate_count_required: "1", yield_currency_value: "16", secondary_currency_value: "1", pity_progress_value: "1" },
  { source_kind: "guardian", rarity: "legendary", duplicate_count_required: "1", yield_currency_value: "40", secondary_currency_value: "3", pity_progress_value: "1" },
  { source_kind: "relic", rarity: "rare", duplicate_count_required: "1", yield_currency_value: "8", secondary_currency_value: "0", pity_progress_value: "1" },
  { source_kind: "relic", rarity: "epic", duplicate_count_required: "1", yield_currency_value: "16", secondary_currency_value: "1", pity_progress_value: "1" },
  { source_kind: "relic", rarity: "legendary", duplicate_count_required: "1", yield_currency_value: "40", secondary_currency_value: "3", pity_progress_value: "1" },
];
const DEFAULT_COLLECTION_FREE_SOURCE_ROWS = [
  { source_id: "col_src_003", name_ko: "일일 공양", reward_kind: "collection_token", reward_ref: "none", reward_qty: "2", unlock_condition: "difficulty>=1" },
  { source_id: "col_src_004", name_ko: "주간 초대", reward_kind: "guardian_pull", reward_ref: "guardian_core", reward_qty: "1", unlock_condition: "difficulty>=32" },
  { source_id: "col_src_008", name_ko: "이벤트 교환", reward_kind: "collection_shard", reward_ref: "none", reward_qty: "30", unlock_condition: "difficulty>=70" },
];
...
const COLLECTION_GUARDIAN_RARITY_BY_ID = {
  gdn_001: "rare",
  gdn_002: "rare",
  gdn_003: "rare",
};
const COLLECTION_RELIC_RARITY_BY_ID = {
  rlc_001: "rare",
  rlc_002: "rare",
  rlc_003: "rare",
};
...
const COLLECTION_DUPLICATE_RULES = {
  guardian: {
    rare: { required: 1, shard: 8, token: 0, pity: 1 },
    epic: { required: 1, shard: 16, token: 1, pity: 1 },
    legendary: { required: 1, shard: 40, token: 3, pity: 1 },
  },
  relic: {
    rare: { required: 1, shard: 8, token: 0, pity: 1 },
    epic: { required: 1, shard: 16, token: 1, pity: 1 },
...
function parseCollectionUnlockDifficulty(unlockCondition, fallback = 1) {
  const matched = String(unlockCondition || "").match(/difficulty>=([0-9]+)/);
...
function buildCollectionCatalog(
  ...
  const guardianList = guardianRows.map((row) => ({
    ...
    nameKo: row.name_ko,
    rarity: row.rarity,
    effectLabel: row.note || row.effect_type || "효과 준비 중",
    acquirePool: row.acquire_pool || "guardian_core",
    unlockDifficulty: parseCollectionUnlockDifficulty(row.unlock_condition, 1),
  }));
  const relicList = relicRows.map((row) => ({
    id: row.relic_id,
    nameKo: row.name_ko,
    rarity: row.rarity,
    effectLabel: row.note || row.effect_type || "효과 준비 중",
    acquirePool: row.acquire_pool || "relic_core",
    unlockDifficulty: parseCollectionUnlockDifficulty(row.unlock_condition, 1),
  }));
  const guardiansById = Object.fromEntries(
  ...
  const relicsById = Object.fromEntries(
    relicList.map((relic) => [
      relic.id,
      {
        ...relic,
  ...
  const poolsById = {};
  for (const guardian of Object.values(guardiansById)) {
    poolsById[guardian.acquirePool] = poolsById[guardian.acquirePool] || [];
    poolsById[guardian.acquirePool].push(guardian.id);
  }
  for (const relic of Object.values(relicsById)) {
    poolsById[relic.acquirePool] = poolsById[relic.acquirePool] || [];
    poolsById[relic.acquirePool].push(relic.id);
  }
  const duplicateRules = { guardian: {}, relic: {} };
  for (const row of duplicateRows) {
    const kind = String(row.source_kind || "").trim();
    const rarity = String(row.rarity || "").trim();
    if (!kind || !rarity) {
      continue;
    }
    duplicateRules[kind] = duplicateRules[kind] || {};
    duplicateRules[kind][rarity] = {
      required: Math.max(1, Math.floor(Number(row.duplicate_count_required) || 1)),
      shard: Math.max(0, Math.floor(Number(row.yield_currency_value) || 0)),
      token: Math.max(0, Math.floor(Number(row.secondary_currency_value) || 0)),
...
function ensureCollectionStateShape() {
  ...
  if (
    !state.collection.guardianDuplicateCounts ||
    typeof state.collection.guardianDuplicateCounts !== "object"
  ) {
    state.collection.guardianDuplicateCounts = {};
  }
  if (
    !state.collection.relicDuplicateCounts ||
    typeof state.collection.relicDuplicateCounts !== "object"
  ) {
    state.collection.relicDuplicateCounts = {};
  }
  state.collection.guardianDuplicateCounts = Object.fromEntries(
    Object.entries(state.collection.guardianDuplicateCounts)
      .filter(([key]) => typeof key === "string" && key.trim())
      .map(([key, value]) => [key.trim(), Math.max(0, Math.floor(Number(value) || 0))]),
  );
  state.collection.relicDuplicateCounts = Object.fromEntries(
    Object.entries(state.collection.relicDuplicateCounts)
      .filter(([key]) => typeof key === "string" && key.trim())
      .map(([key, value]) => [key.trim(), Math.max(0, Math.floor(Number(value) || 0))]),
  );
...
function resolveCollectionGuardianName(id) {
  return (
    collectionCatalog?.guardiansById?.[id]?.nameKo ||
    COLLECTION_GUARDIAN_NAME_BY_ID[id] ||
    id ||
    "호법 미선택"
  );
}
...
function resolveCollectionRelicName(id) {
  return (
    collectionCatalog?.relicsById?.[id]?.nameKo ||
    COLLECTION_RELIC_NAME_BY_ID[id] ||
    id ||
    "법보 미선택"
  );
}

function resolveCollectionGuardianEffectLabel(id) {
  return (
    collectionCatalog?.guardiansById?.[id]?.effectLabel ||
    COLLECTION_GUARDIAN_EFFECT_LABEL_BY_ID[id] ||
    "전투/돌파/운영 보조 효과"
  );
}

function resolveCollectionRelicEffectLabel(id) {
  return (
    collectionCatalog?.relicsById?.[id]?.effectLabel ||
    COLLECTION_RELIC_EFFECT_LABEL_BY_ID[id] ||
    "규칙 변형/준비 선택지 확장 효과"
  );
}
...
function resolveCollectionGuardianRarity(id) {
  return (
    collectionCatalog?.guardiansById?.[id]?.rarity ||
    COLLECTION_GUARDIAN_RARITY_BY_ID[id] ||
    "rare"
  );
}
...
function resolveCollectionRelicRarity(id) {
  return (
    collectionCatalog?.relicsById?.[id]?.rarity ||
    COLLECTION_RELIC_RARITY_BY_ID[id] ||
    "rare"
  );
}

function resolveCollectionRarityLabel(rarity) {
  if (rarity === "legendary") {
    return "전설";
  }
  if (rarity === "epic") {
...
function renderCollectionChoiceGrid(
  node,
  definitions,
  { kind, ownedIds, selectedId, currentDifficultyIndex },
) {
  if (!node) {
    return;
  }
  ...
  for (const definition of definitions) {
    const owned = ownedIds.includes(definition.id);
    const selected = selectedId === definition.id;
    const unlocked = currentDifficultyIndex >= definition.unlockDifficulty;
    const article = document.createElement("article");
    article.className = "collection-card collection-card-choice";
    article.setAttribute("role", "button");
    article.setAttribute("tabindex", "0");
    article.setAttribute("aria-pressed", String(selected));
    if (kind === "guardian") {
      article.dataset.guardianId = definition.id;
    } else {
      article.dataset.relicId = definition.id;
    }
    article.dataset.owned = String(owned);
    article.dataset.selected = String(selected);
    article.dataset.unlocked = String(unlocked);
    article.dataset.rarity = definition.rarity || "rare";
    article.title = `${definition.nameKo} · ${resolveCollectionRarityLabel(definition.rarity)} · ${definition.effectLabel}`;

    const title = document.createElement("strong");
    title.textContent = definition.nameKo;

    const meta = document.createElement("div");
    meta.className = "collection-card-meta";

    const rarityChip = document.createElement("span");
    rarityChip.className = "collection-card-chip";
    rarityChip.textContent = resolveCollectionRarityLabel(definition.rarity);

    const stateChip = document.createElement("span");
    stateChip.className = "collection-card-chip";
    stateChip.dataset.tone = !unlocked ? "warn" : owned ? "success" : "info";
    stateChip.textContent = !unlocked
      ? `난이도 ${definition.unlockDifficulty}`
      : owned
        ? "보유"
        : "미보유";

    meta.append(rarityChip, stateChip);

    const effect = document.createElement("span");
    effect.className = "sub";
    effect.textContent = definition.effectLabel;

    article.append(title, meta, effect);
...
function grantCollectionPull(collection, poolId) {
  const pool =
    collectionCatalog?.poolsById?.[poolId] || COLLECTION_POOL_BY_ID[poolId] || [];
  ...
  if (nextId) {
    ...
    if (isGuardianPool) {
      ...
      return `${resolveCollectionGuardianName(nextId)} 확보`;
    }
    collection.selectedRelicId = nextId;
    return `${resolveCollectionRelicName(nextId)} 확보`;
  }
  const duplicateId = pool[0] || "";
  if (!duplicateId) {
    return "획득 가능한 수집 대상 없음";
  }
  if (isGuardianPool) {
    collection.guardianDuplicateCounts[duplicateId] =
      Math.max(0, Math.floor(Number(collection.guardianDuplicateCounts[duplicateId]) || 0)) + 1;
    return `${resolveCollectionGuardianName(duplicateId)} 중복 +1`;
  }
  collection.relicDuplicateCounts[duplicateId] =
    Math.max(0, Math.floor(Number(collection.relicDuplicateCounts[duplicateId]) || 0)) + 1;
  return `${resolveCollectionRelicName(duplicateId)} 중복 +1`;
}

function resolveCollectionDuplicateRule(kind, rarity) {
  return (
    collectionCatalog?.duplicateRules?.[kind]?.[rarity] ||
    COLLECTION_DUPLICATE_RULES[kind]?.[rarity] ||
    null
  );
}
...
function resolveCollectionDuplicateCount(collection, kind, id) {
  const source =
    kind === "guardian" ? collection.guardianDuplicateCounts : collection.relicDuplicateCounts;
...
function syncCollectionPanel() {
  ...
  const guardianPrimaryLabel = collection.guardianLoadout.primary
    ? resolveCollectionGuardianName(collection.guardianLoadout.primary)
    : "비어 있음";
  const guardianSecondaryLabel = collection.guardianSlotSecondaryUnlocked
    ? collection.guardianLoadout.secondary
      ? resolveCollectionGuardianName(collection.guardianLoadout.secondary)
      : "비어 있음"
    : "잠금";
  const relicPrimaryLabel = collection.relicLoadout.primary
    ? resolveCollectionRelicName(collection.relicLoadout.primary)
    : "비어 있음";
  const relicSecondaryLabel = collection.relicSlotSecondaryUnlocked
    ? collection.relicLoadout.secondary
      ? resolveCollectionRelicName(collection.relicLoadout.secondary)
      : "비어 있음"
    : "잠금";
  ...
  const selectedRelicDuplicateCount = resolveCollectionDuplicateCount(
    collection,
    "relic",
    selectedRelicId,
  );
  const selectedGuardianRule = resolveCollectionDuplicateRule(
    "guardian",
    resolveCollectionGuardianRarity(selectedGuardianId),
  );
  const selectedRelicRule = resolveCollectionDuplicateRule(
    "relic",
    resolveCollectionRelicRarity(selectedRelicId),
  );
  ...
  dom.collectionPanel.dataset.duplicateSummary =
    `호법 중복 ${selectedGuardianDuplicateCount} · 법보 중복 ${selectedRelicDuplicateCount} · 파편 ${collection.shard} · 토큰 ${collection.token} · 천장 ${collection.pityProgress}/${collection.pityMax}`;
  ...
  if (dom.btnConvertGuardianDuplicate) {
    const canConvert = !!selectedGuardianRule &&
      selectedGuardianDuplicateCount >= selectedGuardianRule.required;
    dom.btnConvertGuardianDuplicate.disabled = !canConvert;
    dom.btnConvertGuardianDuplicate.title = canConvert
      ? `파편 +${selectedGuardianRule.shard} · 토큰 +${selectedGuardianRule.token} · 천장 +${selectedGuardianRule.pity}`
      : "전환 가능한 호법 중복 없음";
  }
  if (dom.btnConvertRelicDuplicate) {
    const canConvert = !!selectedRelicRule &&
      selectedRelicDuplicateCount >= selectedRelicRule.required;
    dom.btnConvertRelicDuplicate.disabled = !canConvert;
    dom.btnConvertRelicDuplicate.title = canConvert
      ? `파편 +${selectedRelicRule.shard} · 토큰 +${selectedRelicRule.token} · 천장 +${selectedRelicRule.pity}`
      : "전환 가능한 법보 중복 없음";
  }
  ...
  renderCollectionChoiceGrid(
    dom.guardianRosterGrid,
    collectionCatalog?.guardianList || buildCollectionCatalog().guardianList,
    {
      kind: "guardian",
      ownedIds: collection.guardianOwnedIds,
      selectedId: selectedGuardianId,
      currentDifficultyIndex,
    },
  );
  renderCollectionChoiceGrid(
    dom.relicArchiveGrid,
    collectionCatalog?.relicList || buildCollectionCatalog().relicList,
    {
      kind: "relic",
      ownedIds: collection.relicOwnedIds,
      selectedId: selectedRelicId,
      currentDifficultyIndex,
    },
  );
  if (dom.guardianDetailName) {
    dom.guardianDetailName.textContent =
      selectedGuardianId ? resolveCollectionGuardianName(selectedGuardianId) : "호법 상세 대기";
  }
  if (dom.guardianDetailEffect) {
    dom.guardianDetailEffect.textContent =
      `${resolveCollectionGuardianEffectLabel(selectedGuardianId)} · 중복 ${selectedGuardianDuplicateCount}개`;
  }
  if (dom.relicDetailName) {
    dom.relicDetailName.textContent =
      selectedRelicId ? resolveCollectionRelicName(selectedRelicId) : "법보 상세 대기";
  }
  if (dom.relicDetailEffect) {
    dom.relicDetailEffect.textContent =
      `${resolveCollectionRelicEffectLabel(selectedRelicId)} · 중복 ${selectedRelicDuplicateCount}개`;
  }
...
function resolveOpsDigestToplineSourceClusterTone(sourceTone, freshnessTone) {
  const normalizedSourceTone = String(sourceTone || "info").trim() || "info";
  const normalizedFreshnessTone = String(freshnessTone || "info").trim() || "info";
  if (normalizedSourceTone === "error" || normalizedSourceTone === "warn") {
    return normalizedSourceTone;
  }
  if (normalizedFreshnessTone === "warn") {
    return "warn";
  }
  if (normalizedSourceTone === "success" || normalizedFreshnessTone === "success") {
    return "success";
  }
  return "info";
}

function resolveOpsDigestToplineRecentClusterTone(
  recentTone,
  sourceClusterTone,
  freshnessTone,
) {
  const normalizedRecentTone = String(recentTone || "info").trim() || "info";
  const normalizedSourceClusterTone =
    String(sourceClusterTone || "info").trim() || "info";
  const normalizedFreshnessTone = String(freshnessTone || "info").trim() || "info";
  if (normalizedRecentTone === "error" || normalizedSourceClusterTone === "error") {
    return "error";
  }
  if (
    normalizedRecentTone === "warn" ||
    normalizedSourceClusterTone === "warn" ||
    normalizedFreshnessTone === "warn"
  ) {
    return "warn";
  }
  if (
    normalizedRecentTone === "success" ||
    normalizedSourceClusterTone === "success" ||
    normalizedFreshnessTone === "success"
  ) {
    return "success";
  }
  return "info";
...
function formatOpsDigestFilterChipLabel(kind, label) {
  ...
  switch (normalizedKind) {
    case "source":
      return normalizedLabel.replace(/^출처\s+/, "").trim() || "전체";
    case "tone":
      return normalizedLabel === "전체 흐름" ? "전체" : normalizedLabel;
    case "reset":
      return normalizedLabel.replace(/^필터\s+/, "").trim() || "해제";
    default:
      return normalizedLabel.replace(/\s*흐름$/, "").trim() || normalizedLabel;
  }
}
...
function syncOpsDigestTriageStrip() {
  ...
  if (dom.opsDigestToplineSource) {
    applyRiskTone(dom.opsDigestToplineSource, toplineSourceClusterTone);
  }
  if (dom.btnOpsDigestToplineSourceJump) {
    applyRiskTone(dom.btnOpsDigestToplineSourceJump, toplineSourceClusterTone);
  }
...
function renderBattleSceneDuelHud() {
  ...
  if (dom.battleScenePlayerHpBar) {
    ...
    dom.battleScenePlayerHpBar.dataset.overviewSummary =
      `HP ${playerHpPct}% · ${playerHpTier} · hp_${playerHpTier}`;
  }
  if (dom.battleSceneEnemyHpBar) {
    ...
    dom.battleSceneEnemyHpBar.dataset.overviewSummary =
      `HP ${enemyHpPct}% · ${enemyHpTier} · hp_${enemyHpTier}`;
  }
  if (dom.battleScenePlayerCastBar) {
    ...
    dom.battleScenePlayerCastBar.dataset.overviewSummary =
      `기세 ${playerCastPct}% · ${playerCastTier} · cast_${playerCastTier}`;
  }
  if (dom.battleSceneEnemyCastBar) {
    ...
    dom.battleSceneEnemyCastBar.dataset.overviewSummary =
      `기세 ${enemyCastPct}% · ${enemyCastTier} · cast_${enemyCastTier}`;
  }
  ...
  if (dom.battleScenePlayer) {
    ...
    dom.battleScenePlayer.dataset.overviewSummary =
      `${dom.battleScenePlayer.dataset.actorFrame || "idle"} · HP ${playerHpPct}% · 기세 ${playerCastPct}% · ${playerHpTier}_${playerCastTier}`;
    if (dom.battleSceneArena) {
  ...
  if (dom.battleSceneEnemy) {
    ...
    dom.battleSceneEnemy.dataset.overviewSummary =
      `${dom.battleSceneEnemy.dataset.actorFrame || "idle"} · HP ${enemyHpPct}% · 기세 ${enemyCastPct}% · ${enemyHpTier}_${enemyCastTier}`;
    if (dom.battleSceneArena) {
...
function bindEvents() {
  ...
  for (const button of [
    ...
    button?.addEventListener("click", () => {
          ...
          collection.guardianOwnedIds.length > 0 && selectedId
            ? (button === dom.btnEquipGuardianPrimary
                ? ((collection.guardianLoadout.primary = selectedId), `${label}: ${
                    resolveCollectionGuardianName(selectedId)
                  }`)
      ...
      if (button === dom.btnConvertGuardianDuplicate) {
        const selectedId = collection.selectedGuardianId || "";
        const rule = resolveCollectionDuplicateRule(
          "guardian",
          resolveCollectionGuardianRarity(selectedId),
        );
        const duplicateCount = resolveCollectionDuplicateCount(collection, "guardian", selectedId);
        if (!selectedId || !rule || duplicateCount < rule.required) {
          setStatus("전환 가능한 호법 중복 없음", true);
          return;
        }
        collection.guardianDuplicateCounts[selectedId] = Math.max(
          0,
          duplicateCount - rule.required,
        );
        collection.shard += rule.shard;
        collection.token += rule.token;
        collection.pityProgress = Math.min(collection.pityMax, collection.pityProgress + rule.pity);
        persistLocal();
        setStatus(
          `${resolveCollectionGuardianName(selectedId)} 중복 전환 · 파편 +${rule.shard} · 토큰 +${rule.token} · 천장 +${rule.pity}`,
        );
        render();
        return;
      }
          ...
          collection.relicOwnedIds.length > 0 && selectedId
            ? (button === dom.btnEquipRelicPrimary
                ? ((collection.relicLoadout.primary = selectedId), `${label}: ${
                    resolveCollectionRelicName(selectedId)
                  }`)
      ...
      if (button === dom.btnConvertRelicDuplicate) {
        const selectedId = collection.selectedRelicId || "";
        const rule = resolveCollectionDuplicateRule(
          "relic",
          resolveCollectionRelicRarity(selectedId),
        );
        const duplicateCount = resolveCollectionDuplicateCount(collection, "relic", selectedId);
        if (!selectedId || !rule || duplicateCount < rule.required) {
          setStatus("전환 가능한 법보 중복 없음", true);
          return;
        }
        ...
        persistLocal();
        setStatus(
          `${resolveCollectionRelicName(selectedId)} 중복 전환 · 파편 +${rule.shard} · 토큰 +${rule.token} · 천장 +${rule.pity}`,
        );
        render();
      }
    });
  ...
  dom.guardianRosterGrid?.addEventListener("click", (event) => {
    ...
    const card = event.target.closest("[data-guardian-id]");
  ...
  dom.guardianRosterGrid?.addEventListener("keydown", (event) => {
    if (!(event.target instanceof Element)) {
      return;
    }
    if (event.key !== "Enter" && event.key !== " ") {
  ...
  dom.relicArchiveGrid?.addEventListener("click", (event) => {
    ...
    const card = event.target.closest("[data-relic-id]");
    if (!(card instanceof HTMLElement)) {
      return;
    }
  ...
  dom.relicArchiveGrid?.addEventListener("keydown", (event) => {
    if (!(event.target instanceof Element)) {
      return;
    }
    if (event.key !== "Enter" && event.key !== " ") {
