...
- 현재 노드가 `elite/boss/tribulation`이면 대표 몬스터 이름과 기믹 힌트가 같이 보여야 한다.
- 기믹 힌트는 raw code가 아니라 사람이 읽는 한글 설명이어야 한다.
- 현재 노드의 `drop_group` 기반 전리품 성격도 같이 보여야 한다.
- 가능하면 대표 전리품 2~3개도 함께 보여야 한다.
- 가능하면 `핵심 드랍 badge`도 따로 보여서 보스 코어/핵심 소모품이 먼저 읽히게 해야 한다.
