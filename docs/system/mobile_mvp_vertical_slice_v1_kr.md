...
```
  ...
  - 결과 우선 구간에서 result-driven ambient impact가 source-aware replay 예산(`battle max=2`, `breakthrough max=4`, default `3`)과 최소 간격(`960ms`)을 초과하지 않고, 결과 신호가 살아있는 동안에는 lock=`result` 상태로 랜덤 fallback이 차단되다가 explicit 이벤트로 신호가 오래되면 fresh=`stale`로 전환되어 재생이 중단되고, 유휴 전환 시 lock=`free` + fresh=`none` + replay 카운터 0으로 초기화되는지 검증
  - 기 부족 돌파 차단
  - 사망 실패 → 환생 루프 발동
