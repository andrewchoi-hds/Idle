...
- 전투 연출 패널:
  ...
  - ambient impact 경로에 result-driven 우선 규칙(`BATTLE_SCENE_RESULT_DRIVEN_AMBIENT_IMPACT_PRIORITY_WINDOW_MS=6800ms`)을 추가해 최근 엔진 결과가 있으면 해당 결과 cue를 `syncDuel:false`로 재반영하고, 결과 신호가 살아있는 동안에는 랜덤 ambient impact를 잠금(`sceneAmbientImpactLock=result`)으로 차단하며 source-aware replay 예산(`battle=2`, `breakthrough=4`, default `3`) + 최소 간격(`BATTLE_SCENE_RESULT_DRIVEN_AMBIENT_IMPACT_MIN_INTERVAL_MS=960`)을 적용해 재반영 폭주를 제한하고, explicit 이벤트 timestamp+sequence 기준 freshness(`fresh/stale/none`)를 함께 계측해 오래된 결과 신호 재생을 차단하며 arena dataset(`data-scene-ambient-impact=result/random/idle`, `data-scene-ambient-impact-lock=free|result`, `data-scene-ambient-impact-fresh=fresh|stale|none`, `data-scene-ambient-impact-explicit-seq`, `data-scene-ambient-impact-signal-seq`, `data-scene-ambient-impact-source`, `data-scene-ambient-impact-kind`, `data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`)에 출처/잠금/freshness/sequence/신호/replay 상태를 기록
