...
- 전투 연출 패널:
  ...
  - stage 메타(`world`, `difficulty_index`, `is_tribulation`)를 연출 패널 `data-scene-world`, `data-scene-tier`로 반영하고, stage 패널 텍스트 노드(`stageDisplay`, `worldTag`, `difficultyIndex`, `qiRequired`)에도 `data-stage-name`, `data-world-label`, `data-difficulty-index`, `data-qi-required` contract를 유지하며, 루프/성능/결과 우선 모드도 아레나 루트 dataset(`data-scene-loop`, `data-scene-performance`, `data-scene-outcome-priority`)에 기록해 인간계/신선계/진선계 및 난도 구간별 색감·오라 강도와 결과 우선 상태를 동시에 추적 가능하게 유지
