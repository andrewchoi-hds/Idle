...
```
  ...
  - ambient impact 데이터셋이 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind(`data-scene-ambient-impact-kind`)를 함께 노출해, 결과 재사용/랜덤 fallback/유휴 전환이 DOM 레벨에서 추적 가능한지 검증
  - 기 부족 돌파 차단
  - 사망 실패 → 환생 루프 발동
