...
- UI:
  ...
  - `btnExportOfflineReport`로 정산 리포트를 JSON 형태로 `savePayload`에 주입(클립보드 복사 시도)
    ...
    - 내보내기 시점 필터 스냅샷(`detailViewSnapshotAtExport`: 현재 모드/표시·숨김 건수/모드별 라벨)을 함께 기록
...
- 점검 시나리오:
  ...
  - 오프라인 리포트 스냅샷(`buildOfflineDetailReportSnapshot`)이 필터 집계/라벨/숨김 상위 종류 및 내보내기 모드(view) 메타를 일관되게 반환하는지 검증
