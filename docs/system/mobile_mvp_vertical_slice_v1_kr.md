...
```
  ...
  - 결과 우선 구간에서 result-driven ambient impact가 replay 예산(`max=3`)과 최소 간격(`960ms`)을 초과하지 않고, 우선 구간에는 랜덤 fallback이 재생되지 않다가 유휴 전환 시 replay 카운터가 0으로 초기화되는지 검증
  - 기 부족 돌파 차단
  - 사망 실패 → 환생 루프 발동
