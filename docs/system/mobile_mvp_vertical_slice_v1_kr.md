...
```
  ...
  - ambient impact 데이터셋이 잠금 상태(`data-scene-ambient-impact-lock=free|result`), gate 상태(`data-scene-ambient-impact-gate=fresh|stale_sequence|stale_window|replay_exhausted|replay_cooldown|no_signal`), freshness 상태(`data-scene-ambient-impact-fresh=fresh|stale|none`), sequence 상태(`data-scene-ambient-impact-explicit-seq`, `data-scene-ambient-impact-signal-seq`), 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind(`data-scene-ambient-impact-kind`)를 함께 노출하고, replay 계측(`data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`)이 결과 재사용/랜덤 fallback/유휴 전환 경로에서 일관되게 갱신되는지 검증
  ...
  - 결과 우선 구간에서 result-driven ambient impact가 source-aware replay 예산(`battle max=2`, `breakthrough max=4`, default `3`)과 최소 간격(`960ms`)을 초과하지 않고, 결과 신호가 살아있는 동안에는 lock=`result` 상태로 랜덤 fallback이 차단되다가 explicit 이벤트 sequence가 증가하면 fresh=`stale`로 전환되어(동일 ms 연속 이벤트 포함) 재생이 중단되고, 유휴 전환 시 lock=`free` + fresh=`none` + replay 카운터 0으로 초기화되는지 검증
  - 기 부족 돌파 차단
  - 사망 실패 → 환생 루프 발동
