...
```
  ...
  - ambient impact 데이터셋이 잠금 상태(`data-scene-ambient-impact-lock=free|result`), gate 상태(`data-scene-ambient-impact-gate=fresh|stale_sequence|stale_window|replay_exhausted|replay_cooldown|no_signal`), freshness/active 상태(`data-scene-ambient-impact-fresh=fresh|stale|none`, `data-scene-ambient-impact-active=signal|none`), random 상태(`data-scene-ambient-impact-random-state=idle|ready|triggered|suppressed_result_signal|suppressed_replay_cooldown|suppressed_replay_exhausted|suppressed_recovery_window`), random recovery 상태(`data-scene-ambient-impact-random-recovery-source=none|battle|breakthrough`, `data-scene-ambient-impact-random-recovery-ms`, `data-scene-ambient-impact-random-recovery-max-ms`), sequence 상태(`data-scene-ambient-impact-explicit-seq`, `data-scene-ambient-impact-signal-seq`), 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind/outcome(`data-scene-ambient-impact-kind`, `data-scene-ambient-impact-outcome-code`)를 함께 노출하고, replay/cooldown/priority/age 계측(`data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`, `data-scene-ambient-impact-replay-remaining`, `data-scene-ambient-impact-cooldown-ms`, `data-scene-ambient-impact-cooldown-max-ms`, `data-scene-ambient-impact-priority-remaining-ms`, `data-scene-ambient-impact-priority-max-ms`, `data-scene-ambient-impact-signal-age-ms`, `data-scene-ambient-impact-signal-age-max-ms`)이 결과 재사용/랜덤 fallback/유휴 전환 경로에서 일관되게 갱신되는지 검증
