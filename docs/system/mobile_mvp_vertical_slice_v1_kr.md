...
- 전투 연출 패널:
  ...
  - ambient impact 경로에 result-driven 우선 규칙(`BATTLE_SCENE_RESULT_DRIVEN_AMBIENT_IMPACT_PRIORITY_WINDOW_MS=6800ms`)을 추가해 최근 엔진 결과가 있으면 해당 결과 cue를 `syncDuel:false`로 재반영하고, 결과 신호가 살아있는 동안에는 랜덤 ambient impact를 잠금(`sceneAmbientImpactLock=result`)으로 차단하며 source-aware replay 예산(`battle=2`, `breakthrough=4`, default `3`) + source-aware 최소 간격(`battle=1080ms`, `breakthrough=840ms`, default `960ms`) + source-aware priority window(`battle=6200ms`, `breakthrough=7600ms`, default `6800ms`)를 적용해 재반영 폭주를 제한하고, explicit 이벤트 timestamp+sequence 기준 freshness(`fresh/stale/none`)를 함께 계측해 오래된 결과 신호 재생을 차단하며 gate reason(`fresh/stale_sequence/stale_window/replay_exhausted/replay_cooldown/no_signal`)을 노출해 랜덤 fallback 허용/차단 근거를 추적하고 arena dataset(`data-scene-ambient-impact=result/random/idle`, `data-scene-ambient-impact-lock=free|result`, `data-scene-ambient-impact-gate=*`, `data-scene-ambient-impact-fresh=fresh|stale|none`, `data-scene-ambient-impact-explicit-seq`, `data-scene-ambient-impact-signal-seq`, `data-scene-ambient-impact-source`, `data-scene-ambient-impact-kind`, `data-scene-ambient-impact-outcome-code`, `data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`, `data-scene-ambient-impact-replay-remaining`, `data-scene-ambient-impact-cooldown-ms`, `data-scene-ambient-impact-cooldown-max-ms`, `data-scene-ambient-impact-priority-remaining-ms`, `data-scene-ambient-impact-priority-max-ms`, `data-scene-ambient-impact-signal-age-ms`, `data-scene-ambient-impact-signal-age-max-ms`)에 출처/잠금/gate/freshness/sequence/신호/outcome/replay/cooldown/priority/age 상태를 기록
...
```
  ...
  - ambient impact 데이터셋이 잠금 상태(`data-scene-ambient-impact-lock=free|result`), gate 상태(`data-scene-ambient-impact-gate=fresh|stale_sequence|stale_window|replay_exhausted|replay_cooldown|no_signal`), freshness 상태(`data-scene-ambient-impact-fresh=fresh|stale|none`), sequence 상태(`data-scene-ambient-impact-explicit-seq`, `data-scene-ambient-impact-signal-seq`), 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind/outcome(`data-scene-ambient-impact-kind`, `data-scene-ambient-impact-outcome-code`)를 함께 노출하고, replay/cooldown/priority/age 계측(`data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`, `data-scene-ambient-impact-replay-remaining`, `data-scene-ambient-impact-cooldown-ms`, `data-scene-ambient-impact-cooldown-max-ms`, `data-scene-ambient-impact-priority-remaining-ms`, `data-scene-ambient-impact-priority-max-ms`, `data-scene-ambient-impact-signal-age-ms`, `data-scene-ambient-impact-signal-age-max-ms`)이 결과 재사용/랜덤 fallback/유휴 전환 경로에서 일관되게 갱신되는지 검증
