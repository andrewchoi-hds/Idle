...
- 전투 연출 패널:
  ...
  - ambient impact 경로에 result-driven 우선 규칙(`BATTLE_SCENE_RESULT_DRIVEN_AMBIENT_IMPACT_PRIORITY_WINDOW_MS=6800ms`)을 추가해 최근 엔진 결과가 있으면 해당 결과 cue를 `syncDuel:false`로 재반영하고, 결과가 없을 때 랜덤 ambient impact는 cadence 게이트(`BATTLE_SCENE_AMBIENT_RANDOM_IMPACT_DIVISOR=2`)로 빈도를 하향하며 source-aware replay 예산(`battle=2`, `breakthrough=4`, default `3`) + 최소 간격(`BATTLE_SCENE_RESULT_DRIVEN_AMBIENT_IMPACT_MIN_INTERVAL_MS=960`)을 적용해 재반영 폭주를 제한하고 arena dataset(`data-scene-ambient-impact=result/random/idle`, `data-scene-ambient-impact-source`, `data-scene-ambient-impact-kind`, `data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`)에 출처/신호/replay 상태를 기록
...
```
  ...
  - ambient impact 데이터셋이 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind(`data-scene-ambient-impact-kind`)를 함께 노출하고, replay 계측(`data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`)이 결과 재사용/랜덤 fallback/유휴 전환 경로에서 일관되게 갱신되는지 검증
  - 결과 우선 구간에서 result-driven ambient impact가 source-aware replay 예산(`battle max=2`, `breakthrough max=4`, default `3`)과 최소 간격(`960ms`)을 초과하지 않고, 우선 구간에는 랜덤 fallback이 재생되지 않다가 유휴 전환 시 replay 카운터가 0으로 초기화되는지 검증
  - 기 부족 돌파 차단
  - 사망 실패 → 환생 루프 발동
