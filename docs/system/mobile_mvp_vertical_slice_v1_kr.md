...
```
  ...
  - ambient impact 데이터셋이 잠금 상태(`data-scene-ambient-impact-lock=free|result`), gate 상태(`data-scene-ambient-impact-gate=fresh|stale_sequence|stale_window|replay_exhausted|replay_cooldown|no_signal`), freshness 상태(`data-scene-ambient-impact-fresh=fresh|stale|none`), sequence 상태(`data-scene-ambient-impact-explicit-seq`, `data-scene-ambient-impact-signal-seq`), 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind(`data-scene-ambient-impact-kind`)를 함께 노출하고, replay/cooldown/priority 계측(`data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`, `data-scene-ambient-impact-replay-remaining`, `data-scene-ambient-impact-cooldown-ms`, `data-scene-ambient-impact-cooldown-max-ms`, `data-scene-ambient-impact-priority-remaining-ms`, `data-scene-ambient-impact-priority-max-ms`)이 결과 재사용/랜덤 fallback/유휴 전환 경로에서 일관되게 갱신되는지 검증
  ...
  - 결과 우선 구간에서 result-driven ambient impact가 source-aware replay 예산(`battle max=2`, `breakthrough max=4`, default `3`)과 source-aware 최소 간격(`battle=1080ms`, `breakthrough=840ms`, default `960ms`)을 초과하지 않고, `replay_cooldown` 구간에서는 cooldown 계측(`cooldown-ms`, `cooldown-max-ms`)이 남은 시간/최대 간격을 반영하며, priority 계측(`priority-remaining-ms`, `priority-max-ms`)이 gate reason 전환(`fresh/replay_* → stale_window`)에 맞춰 감소/종료되고, 결과 신호가 살아있는 동안 lock=`result` 상태로 랜덤 fallback이 차단되다가 explicit 이벤트 sequence가 증가하면 fresh=`stale`로 전환되어(동일 ms 연속 이벤트 포함) 재생이 중단되고, 유휴 전환 시 lock=`free` + fresh=`none` + replay/cooldown/priority 카운터 0으로 초기화되는지 검증
  - 기 부족 돌파 차단
  - 사망 실패 → 환생 루프 발동
