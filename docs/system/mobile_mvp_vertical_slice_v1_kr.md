...
- UI:
  ...
  - `btnExportOfflineReport`로 정산 리포트를 JSON 형태로 `savePayload`에 주입(클립보드 복사 시도)
    ...
    - 리포트에 상세 로그 필터 스냅샷(`detailReportSnapshot`: 전체/핵심 표시건수, 숨김건수, 숨김 상위 종류, kind digest)을 함께 기록
    - 내보내기 시점 필터 스냅샷(`detailViewSnapshotAtExport`: 현재 모드/표시·숨김 건수/모드별 라벨/kind digest)을 함께 기록
...
- 점검 시나리오:
  ...
  - 오프라인 리포트 스냅샷(`buildOfflineDetailReportSnapshot`)이 필터 집계/라벨/숨김 상위 종류 및 kind digest(view 포함)를 일관되게 반환하는지 검증
