...
```
  ...
  - ambient impact 데이터셋이 잠금 상태(`data-scene-ambient-impact-lock=free|result`), freshness 상태(`data-scene-ambient-impact-fresh=fresh|stale|none`), 출처 분해(`data-scene-ambient-impact-source=battle|breakthrough|random|idle`)와 신호 kind(`data-scene-ambient-impact-kind`)를 함께 노출하고, replay 계측(`data-scene-ambient-impact-replay`, `data-scene-ambient-impact-replay-max`)이 결과 재사용/랜덤 fallback/유휴 전환 경로에서 일관되게 갱신되는지 검증
