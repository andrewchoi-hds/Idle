...
DEFAULT_REPORT = ROOT / "data/sim/pr_body_generator_regression_report_v1.json"
DEFAULT_OUTPUT_MD = ROOT / "data/sim/pr_body_generator_regression_ci_summary_v1.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build markdown summary from PR body generator regression report JSON."
    )
...
def build_markdown(report: dict[str, Any]) -> str:
    lines: list[str] = []
    suite_name = safe_text(report.get("suite"), "pr_body_generator_regression")
    ...
    lines.append("## PR Body Generator Regression Summary")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "| Case | Regression Check | Changed Files | Commits | Expected Validation Failures | Error Count |"
    )
    lines.append("| --- | --- | --- | --- | --- | --- |")
    ...
    for row in results:
        ...
        changed_files = safe_int(row.get("changed_file_count"), 0)
        commits = safe_int(row.get("commit_count"), 0)
        expected_failures = safe_int(row.get("expected_validation_failure_count"), 0)
        errors = row.get("errors", [])
        error_count = len(errors) if isinstance(errors, list) else 0
        lines.append(
            "| {case_id} | {result} | {changed_files} | {commits} | {expected_failures} | {error_count} |".format(
                case_id=case_id,
