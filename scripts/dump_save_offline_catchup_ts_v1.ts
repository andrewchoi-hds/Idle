...
import { runSaveOfflineCatchupRuntime } from "../src/progression/saveOfflineCatchupRuntime";
...
function parseArgs(argv: string[]): CliConfig {
  ...
  let outputPath = resolve(
    process.cwd(),
    "data/sim/save_v2_offline_catchup_ts_v1.json",
  );
  let nowEpochMs: number | null = null;
  let anchorEpochMs: number | null = null;
  let maxOfflineHours: number | null = null;
  let tickSec = 5;
  let seed = 20260223;
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (arg === "--now-epoch-ms" && next) {
      nowEpochMs = Math.max(0, parseIntArg(next, 0));
      i += 1;
      continue;
    }
    if (arg === "--anchor-epoch-ms" && next) {
      anchorEpochMs = Math.max(0, parseIntArg(next, 0));
      i += 1;
      continue;
    }
    if (arg === "--max-offline-hours" && next) {
      maxOfflineHours = Math.max(0, parseFloatArg(next, 0));
      i += 1;
      continue;
    }
    if (arg === "--tick-sec" && next) {
      tickSec = Math.max(1, parseIntArg(next, 5));
      i += 1;
      continue;
    }
    if (arg === "--seed" && next) {
...
async function main(): Promise<void> {
  ...
  const result = runSaveOfflineCatchupRuntime(tables, indexes, saveBefore, {
    nowEpochMs: cli.nowEpochMs ?? undefined,
    anchorEpochMs: cli.anchorEpochMs ?? undefined,
    maxOfflineHoursOverride: cli.maxOfflineHours ?? undefined,
    tickSec: cli.tickSec,
    rngSeed: cli.seed,
    ...
    syncTimestampsToNow: !cli.keepTimestamps,
  });
  ...
  process.stdout.write(`wrote save offline catchup report -> ${cli.outputPath}\n`);
  process.stdout.write(
    [
      `raw_offline_sec=${result.summary.rawOfflineSec}`,
      `applied_offline_sec=${result.summary.appliedOfflineSec}`,
      `capped=${result.summary.cappedByMaxOffline}`,
      `final_stage=${result.summary.autoProgressSummary?.finalDifficultyIndex ?? saveBefore.progression.difficulty_index}`,
      `rebirth_delta=${result.summary.autoProgressSummary?.rebirthCountDelta ?? 0}`,
    ].join(" | "),
  );
  process.stdout.write("\n");
}
