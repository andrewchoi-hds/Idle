...
DEFAULT_REPORT = ROOT / "data/sim/pr_body_lint_regression_report_v1.json"
DEFAULT_OUTPUT_MD = ROOT / "data/sim/pr_body_lint_regression_ci_summary_v1.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build markdown summary from PR body lint regression report JSON."
    )
...
def build_markdown(report: dict[str, Any]) -> str:
    lines: list[str] = []
    suite_name = safe_text(report.get("suite"), "pr_body_lint_regression")
    ...
    lines.append("## PR Body Lint Regression Summary")
    lines.append("")
    ...
    lines.append("### Cases")
    lines.append("")
    lines.append("| Case | Expected Lint | Regression Check | Input | Strict Path | Error Count |")
    lines.append("| --- | --- | --- | --- | --- | --- |")
    ...
    for row in results:
        ...
        input_type = safe_text(row.get("input_type"))
        strict_path = safe_text(row.get("strict_change_path"))
        errors = row.get("errors", [])
        error_count = len(errors) if isinstance(errors, list) else 0
        lines.append(
            "| {case_id} | {expected} | {result} | {input_type} | {strict_path} | {error_count} |".format(
                case_id=case_id,
